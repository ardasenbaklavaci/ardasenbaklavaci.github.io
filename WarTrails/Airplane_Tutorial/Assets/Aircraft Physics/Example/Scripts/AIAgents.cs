...
public class AIAgents : Agent
{
    ...
    /// </summary>
    public override void Initialize() // ! //initialise and boot ... 
    {
        ...
        //Max 5000 step if training, infinite steps if racing
        MaxStep = area.trainingMode ? 5000 : 0;
    }
    public override void CollectObservations(VectorSensor sensor) // obstacles... , unity ML 's observation sensor...
    {
    ...
    {
        Vector3 nextCheckpointDir = area.Checkpoints[NextCheckpointIndex].transform.position - transform.position; // !!!!!!!!!!! directory difference for movement...
        Vector3 localCheckpointDir = transform.InverseTransformDirection(nextCheckpointDir);
        return localCheckpointDir;
    }

    private void GotCheckpoint() // 2 calls  // !!! 
    {
    ...
    public override void OnEpisodeBegin() // ! boot 
    {
        //Reset the velocity,position and orientation; // airfield's location as starting cordinates (vector)
    ...
    private void OnTriggerEnter(Collider other) // !!! // checkpoint reached ... 
    {
